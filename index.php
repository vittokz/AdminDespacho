<?php require_once("seguridad/seguridad.php");
    ...
    $hoy = date("Y-m-d");
    $fechaAnterior = date("Y-m-d",strtotime($hoy."- 4 days")); 
    //verifico si existen procesos a vencer contrato
    $sqlP=mysql_query("SELECT * FROM des_procesos where estado like 'Activo' and fechaTerminacion between '$fechaAnterior' and '$hoy'  order by fechaProceso DESC");
          $registros =0;
...
<?php
 ...
 $hora =date("G");
 $minutos=date("i");
 $horaCompleta2 = $hora.":".$minutos;
 $minutos=$minutos-10;
 $horaCompleta = $hora.":".$minutos;
 $sql="SELECT * FROM des_tareas where fecha between '$hoy' and '$hoy' and hora like '$horaCompleta2' and  usuarioRegistro like '$usuario' order by fecha ASC";
...
<?php
 $hoy = date("Y-m-d");	
 $bandera=0;
$sql=mysql_query("SELECT * FROM des_cotizacion where fechaRegistro like '%$hoy%'");
    while($row=mysql_fetch_array($sql)) 
     {
         $bandera=1;
     }
...
<?php
 $hoy = date("Y-m-d");	
 $banderaL=0;
 $nuevafechaL = strtotime ( '+3 day' , strtotime ( $hoy ) ) ;
 $nuevafechaL = date ( 'Y-m-d' , $nuevafechaL );
 $sql="select * from des_llamadas where fechaVence between '$hoy' and '$nuevafechaL' and usuario like '$usuario' order by fechaVence desc";
...
?>
    <script>
     		Push.create("Notificación  Viacpro", { //Titulo de la notificación
      	body: "Asesor Tiene Pendientes Verifique !!!", //Texto del cuerpo de la notificación
        icon: 'img/mirar.png', //Icono de la notificación
        timeout: 8000, //Tiempo de duración de la notificación
          onClick: function () {//Función que se cumple al realizar clic cobre la notificación
             window.location = "https://www.viacpro.com/AdminDespacho/asesores/asesores.php"; //Redirige a la siguiente web
             this.close(); //Cierra la notificación
          }
        });
    </script>
...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <br><br><br>
      <center><br><br>
     <!-- <a href="alertas/notificaciones.php">
            <img src="img/mensaje.png" title="Enviar notificaciones" style="width:8%;">
            <br>Enviar Notificaciones
      </a> -->
      <div id="resultado">

      </div>
      </center>
      <br><p align="center" style="font-size:34px">
    
        Sistema de Administración
        <small>Control panel</small>
       
      </p>
     <center><img src="img/viacpro.png" style="width:30%"></center>
    </section>
